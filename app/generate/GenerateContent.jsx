...
export default function GenerateContent({ initialHandle }: { initialHandle: string }) {
  ...
  const handleChange = ({ e, index }: { e: React.ChangeEvent<HTMLInputElement>, index?: number }) => {
  ...
  const addLink = () => {
    setForm(prev => ({
      ...prev,
      links: [...prev.links, { id: Date.now(), url: "", text: "" }]
    }));
  };
  ...
  const subitLinks = async () => {
    ...
    try {
      const res = await fetch("/api/add", requestOptions);  // localhost:3000 hataya kyuki production mein issue karega
      ...
      if (result.success) {
        ...
        if (result.data?.findOne?.handle) {
          setGenerated(`${process.env.NEXT_PUBLIC_HOST}/${result.data.findOne.handle}`);
        }
    ...
    <div className="bg-[#E9C0E9] min-h-screen grid grid-cols-2">
          ...
          <div className="item">
            ...
            {form.links.map((item, index) => (
              <div key={item.id} className="mx-4">
                ...
                />
                {form.links.length > 1 && (
                  <button onClick={() => removeLink(item.id)} className="text-red-600 font-bold text-xl">
                    ×
                  </button>
                )}
              </div>
      ...
      <div className="w-full h-screen bg-[#E9C0E9] flex flex-col items-center justify-center">
        ...
        {generated && (
          <Link 
            href={generated}
            className="block flex justify-center items-center text-red-600 hover:text-red-700 font-semibold text-center mt-8 text-xl"
          >
            {generated}
          </Link>
        )}
